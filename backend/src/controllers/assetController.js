...
const { numeric: number, telemetry } = require("../domain/assetTelemetry");
...
function inventorySummary(payload){
  ...
  const total=number(payload.storage?.totalBytes)??(volumes.length?volumes.reduce((sum,v)=>sum+(number(v.sizeBytes)||0),0):null),free=number(payload.storage?.freeBytes)??(volumes.length?volumes.reduce((sum,v)=>sum+(number(v.freeBytes)||0),0):null);
...
async function reportInventory(req,res){
  ...
  try{await client.query("BEGIN");
    ...
    const measured=telemetry(payload);
    const diskUsage=measured.disk;
    const status=alerts.length || [measured.cpu,measured.ram,measured.disk].some(value=>value!==null&&value>=85) || measured.antivirus===false ? "warning" : "online";
    await client.query(`UPDATE ativos SET hostname=COALESCE($1,hostname),fabricante=$2,modelo=$3,serial_number=COALESCE($4,serial_number),sistema_operacional=$5,os_build=$6,processador=$7,ram_total_bytes=$8::bigint,ram_total=CASE WHEN $8::bigint IS NULL THEN ram_total ELSE ROUND(($8::numeric/1073741824),2) END,storage_total_bytes=$9::bigint,storage_free_bytes=$10::bigint,armazenamento=CASE WHEN $9::bigint IS NULL THEN armazenamento ELSE ROUND(($9::numeric/1073741824),1)||' GB total · '||ROUND(($10::numeric/1073741824),1)||' GB livres' END,ip=$11,mac=$12,usuario=$13,last_boot=$14,inventory_json=$15::jsonb,schema_version=$16,agente_versao=$17,ultimo_inventario=$18,ultimo_heartbeat=NOW(),disk_usage=$19::numeric,status=$20,cpu_usage=$22::numeric,ram_usage=$23::numeric,uptime_hours=$24::numeric,antivirus_atualizado=$25,firewall_enabled=$26,atualizado_em=NOW() WHERE id=$21`,[summary.hostname,summary.manufacturer,summary.model,summary.serial,summary.os,summary.build,summary.processor,summary.ramBytes,summary.total,summary.free,summary.ip,summary.mac,summary.user,summary.lastBoot,JSON.stringify(payload),Number(payload.schemaVersion),clean(payload.agentVersion,50),payload.collectedAt,diskUsage,status,req.asset.id,measured.cpu,measured.ram,measured.uptime,measured.antivirus,measured.firewall]);
    await client.query(`INSERT INTO ativo_metricas(ativo_id,cpu_usage,ram_usage,disk_usage) VALUES($1,$2,$3,$4)`,[req.asset.id,measured.cpu,measured.ram,measured.disk]);
    await client.query("COMMIT");res.status(201).json({ok:true,reportId:id,snapshotId,changes:changes.length,alerts:alerts.length});
  }catch(error){await client.query("ROLLBACK");throw error}finally{client.release()}
}
function mapAsset(r){const communicationStatus=inventoryDomain.communicationStatus(r.ultimo_heartbeat);const status=communicationStatus==="no_communication"?"offline":ASSET_STATUSES.has(r.status)?r.status:"offline";return {id:String(r.id),deviceId:r.device_id,patrimonio:r.patrimonio||r.serial_number||r.device_id,hostname:r.hostname,fabricante:r.fabricante||null,modelo:r.modelo||null,serialNumber:r.serial_number||null,municipio:r.municipio||"Não informado",unidade:r.unidade||"Não informada",latitude:r.latitude==null?null:Number(r.latitude),longitude:r.longitude==null?null:Number(r.longitude),status,communicationStatus,ip:r.ip||"-",mac:r.mac||"-",usuario:r.usuario||"-",sistemaOperacional:r.sistema_operacional||"-",osBuild:r.os_build||null,processador:r.processador||"-",ramTotal:Number(r.ram_total)||0,ramTotalBytes:r.ram_total_bytes==null?null:Number(r.ram_total_bytes),storageTotalBytes:r.storage_total_bytes==null?null:Number(r.storage_total_bytes),storageFreeBytes:r.storage_free_bytes==null?null:Number(r.storage_free_bytes),armazenamento:r.armazenamento||"-",cpuUsage:number(r.cpu_usage),ramUsage:number(r.ram_usage),diskUsage:number(r.disk_usage),uptimeHours:number(r.uptime_hours),lastBoot:r.last_boot||null,firewallEnabled:r.firewall_enabled,networkType:r.network_type||"Não informado",linkSpeed:r.link_speed||"Não informada",ultimoHeartbeat:r.ultimo_heartbeat,ultimoInventario:r.ultimo_inventario||null,schemaVersion:r.schema_version||null,hasInventory:Boolean(r.inventory_json),agenteInstalado:true,antivirusAtualizado:r.antivirus_atualizado,usuarioVinculado:r.usuario_id?{id:Number(r.usuario_id),nome:r.vinculo_nome||null,email:r.vinculo_email||null}:null,responsavel:"Equipe de Infraestrutura"};}
const ASSET_WITH_USER="SELECT a.*,v.nome AS vinculo_nome,v.email AS vinculo_email,v.foto_perfil AS vinculo_foto FROM ativos a LEFT JOIN usuarios v ON v.id=a.usuario_id";
// mapAsset é síncrono; a foto do responsável exige URL assinada, resolvida aqui.
async function presentAsset(req,row){const asset=mapAsset(row);if(asset.usuarioVinculado)asset.usuarioVinculado.fotoUrl=(await montarUrlFotoPerfil(req,row.usuario_id,row.vinculo_foto))||null;return asset;}
async function loadAsset(id){const result=await pool.query(`${ASSET_WITH_USER} WHERE a.id=$1`,[id]);return result.rows[0]||null;}
async function list(req,res){const result=await pool.query(`${ASSET_WITH_USER} ORDER BY a.municipio,a.hostname`);res.json(await Promise.all(result.rows.map((row)=>presentAsset(req,row))));}
async function detail(req,res){const row=await loadAsset(req.params.id);if(!row)return res.status(404).json({erro:"Ativo não encontrado"});res.json(await presentAsset(req,row));}
...
// Vínculo opcional entre ativo e usuário (1 responsável principal por ativo).
async function setUser(req,res){
  const assetId=Number(req.params.id);
  if(!Number.isInteger(assetId)||assetId<=0)return res.status(404).json({erro:"Ativo não encontrado"});
  const raw=req.body?.usuario_id;
  const userId=raw===undefined||raw===null||raw===""?null:Number(raw);
  if(userId!==null&&(!Number.isInteger(userId)||userId<=0))return res.status(400).json({erro:"Usuário inválido"});
  if(userId!==null){const user=await pool.query("SELECT 1 FROM usuarios WHERE id=$1 AND COALESCE(status,'ativo')='ativo'",[userId]);if(!user.rows[0])return res.status(404).json({erro:"Usuário ativo não encontrado"});}
  const result=await pool.query("UPDATE ativos SET usuario_id=$1,atualizado_em=NOW() WHERE id=$2 RETURNING id",[userId,assetId]);
  if(!result.rows[0])return res.status(404).json({erro:"Ativo não encontrado"});
  res.json(await presentAsset(req,await loadAsset(assetId)));
}
async function assignableUsers(req,res){
  const term=clean(req.query.q,80)||"";
  const like=`%${term.replace(/[\\%_]/g,"\\$&")}%`;
  const result=await pool.query("SELECT id,nome,email,departamento FROM usuarios WHERE COALESCE(status,'ativo')='ativo' AND ($1='' OR nome ILIKE $2 OR email ILIKE $2) ORDER BY nome LIMIT 20",[term,like]);
  res.json(result.rows);
}
// Somente dados de apresentação: sem IP, MAC, token ou inventário técnico.
async function myAssets(req,res){
  const result=await pool.query("SELECT id,hostname,patrimonio,serial_number,device_id,fabricante,modelo,municipio,unidade,status,sistema_operacional,ultimo_heartbeat FROM ativos WHERE usuario_id=$1 ORDER BY hostname",[req.user.id]);
  res.json(result.rows.map(r=>{const communicationStatus=inventoryDomain.communicationStatus(r.ultimo_heartbeat);return {id:String(r.id),hostname:r.hostname,patrimonio:r.patrimonio||r.serial_number||r.device_id,fabricante:r.fabricante||null,modelo:r.modelo||null,municipio:r.municipio||null,unidade:r.unidade||null,sistemaOperacional:r.sistema_operacional||null,status:communicationStatus==="no_communication"?"offline":ASSET_STATUSES.has(r.status)?r.status:"offline",ultimoHeartbeat:r.ultimo_heartbeat};}));
}
module.exports={setUser,assignableUsers,myAssets,enroll,authenticateAgent,heartbeat,reportInventory,list,detail,inventory,history,changes,snapshots,snapshot,alerts,acknowledgeAlert,locations,adminLocations,updateLocation,updateStatus,saveLocation,createInvite};
