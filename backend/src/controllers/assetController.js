...
const ASSET_WITH_USER="SELECT a.*,v.nome AS vinculo_nome,v.email AS vinculo_email FROM ativos a LEFT JOIN usuarios v ON v.id=a.usuario_id";
async function loadAsset(id){const result=await pool.query(`${ASSET_WITH_USER} WHERE a.id=$1`,[id]);return result.rows[0]||null;}
async function list(req,res){const result=await pool.query(`${ASSET_WITH_USER} ORDER BY a.municipio,a.hostname`);res.json(result.rows.map(mapAsset));}
async function detail(req,res){const row=await loadAsset(req.params.id);if(!row)return res.status(404).json({erro:"Ativo não encontrado"});res.json(mapAsset(row));}
...
async function setUser(req,res){
  ...
  res.json(mapAsset(await loadAsset(assetId)));
}
