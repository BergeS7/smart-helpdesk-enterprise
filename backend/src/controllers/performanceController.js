...
exports.technicianDashboard = async (req,res) => {
  try {
    ...
    const profile=await pool.query("SELECT nome,email,departamento FROM usuarios WHERE id=$1",[technicianId]);
    res.json({...score,...details.rows[0],name:profile.rows[0]?.nome||req.user?.nome||"Técnico",email:profile.rows[0]?.email||req.user?.email||"",departamento:profile.rows[0]?.departamento||"",foto_url:montarUrlFotoPerfil(req,technicianId),rating_distribution:distribution.rows,recent_ratings:ratings.rows});
  }
