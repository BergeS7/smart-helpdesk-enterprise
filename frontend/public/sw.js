 ...
 */
const CACHE = "smart-helpdesk-shell-v8";
const BASE = "/suporte";
const HOME = `${BASE}/`;
const SHELL = [HOME, `${BASE}/offline.html`, `${BASE}/manifest.webmanifest`, `${BASE}/pwa-192-v2.png`, `${BASE}/pwa-512-v2.png`];
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(self.registration.showNotification(data.title || "Smart HelpDesk", {
    ...
    icon: `${BASE}/pwa-192-v2.png`,
    badge: `${BASE}/favicon.png`,
    tag: data.tag || "smart-helpdesk",
    data: { url: data.url || HOME, userId: data.userId },
  }));
});
...
self.addEventListener("notificationclick", (event) => {
  ...
  event.waitUntil((async () => {
    ...
    try {
      ...
      if (candidate.origin === self.location.origin && candidate.pathname === HOME) target = candidate;
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.origin !== self.location.origin || url.pathname.startsWith(`${BASE}/api/`) || url.pathname.startsWith(`${BASE}/uploads/`)) return;

  if (request.mode === "navigate") {
    event.respondWith(fetch(request).then((response) => {
      if (response.ok) caches.open(CACHE).then((cache) => cache.put(HOME, response.clone()));
      return response;
    }).catch(async () => (await caches.match(HOME)) || caches.match(`${BASE}/offline.html`)));
    return;
  }
  ...
  if (!url.pathname.startsWith(`${BASE}/assets/`) && /\.(?:png|jpg|jpeg|webp|svg|ico|woff2?)$/i.test(url.pathname)) {
