...
const documents: Record<LegalDocument,{title:string;sections:Array<[string,string]>}> = {
  privacy:{title:"Política de Privacidade",sections:[
    ["Controladora e contato","A organização que disponibiliza o Smart HelpDesk aos usuários define as finalidades e os meios do tratamento. A identificação completa da controladora, o endereço e o canal de privacidade devem ser fornecidos pela organização antes do uso em produção. Solicitações podem ser encaminhadas ao administrador responsável pela privacidade."],
    ...
    ["Bases e direitos","A controladora deve documentar a base legal de cada finalidade; o aceite dos termos não constitui consentimento geral. O titular pode pedir confirmação, acesso, correção, informações sobre compartilhamento, oposição, anonimização, bloqueio ou eliminação quando cabíveis e revisão de decisões exclusivamente automatizadas. Consentimento, quando utilizado, pode ser revogado."],
    ["Retenção e segurança","Métricas técnicas detalhadas são eliminadas após 90 dias. Os prazos dos demais registros dependem de aprovação da controladora; eliminação ou anonimização deve observar as hipóteses legais de conservação. O acesso é controlado por perfil e permissão."],
    ["Compartilhamentos","O mapa utiliza OpenStreetMap e dados geográficos do IBGE, que podem receber dados técnicos da conexão. Provedores de infraestrutura, e-mail e notificações push podem receber dados necessários quando os respectivos recursos forem usados. A controladora deve informar os operadores efetivos e avaliar transferências internacionais."],
  ]},
  cookies:{title:"Cookies e armazenamento local",sections:[
    ["Uso atual","Não identificamos cookies próprios de publicidade ou análise comportamental. O cookie sidebar_state guarda a barra lateral aberta ou fechada por até 7 dias; ele é enviado ao servidor nas requisições do mesmo domínio."],
    ["Armazenamento local","O navegador guarda token e perfil da sessão até o logout ou limpeza, além de tema, filtros, preferências, estado do aviso e identificação local de notificações push. Dados temporários de recuperação da interface ficam no armazenamento da aba. O token é enviado à API para autenticação."],
    ["Controle","É possível apagar os dados do site nas configurações do navegador. Isso encerra a sessão e redefine preferências; bloquear o armazenamento essencial pode impedir o uso do sistema. Não há categorias opcionais de publicidade ou análise a consentir nesta versão."],
  ]},
...
export function LegalComplianceLayer(){
  ...
  return <>
    ...
    {open&&<div className="fixed inset-0 z-[140] grid place-items-center bg-slate-950/55 p-4" onMouseDown={(e)=>e.target===e.currentTarget&&setOpen(null)}><section role="dialog" aria-modal="true" aria-label={documents[open].title} className="max-h-[88vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl"><header className="flex items-start justify-between gap-3 border-b pb-4"><div><p className="text-[10px] font-black uppercase tracking-widest text-blue-600">Smart HelpDesk · resumo informativo</p><h2 className="mt-1 text-2xl font-black text-zinc-900">{documents[open].title}</h2></div><button type="button" aria-label="Fechar documento" onClick={()=>setOpen(null)}><X/></button></header><div className="space-y-5 py-5">{documents[open].sections.map(([title,text])=><article key={title}><h3 className="font-black text-zinc-800">{title}</h3><p className="mt-1 text-sm leading-6 text-zinc-600">{text}</p></article>)}</div><footer className="border-t pt-4 text-xs leading-5 text-zinc-500"><FileText size={14} className="mr-1 inline"/>Resumo informativo. A organização deve disponibilizar a política integral, sua identificação e o canal de privacidade aos titulares.</footer></section></div>}
  </>;
}
