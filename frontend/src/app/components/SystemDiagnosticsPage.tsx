...
export function SystemDiagnosticsPage({ dark }: { dark: boolean }) {
  ...
  const cards=[
    ...
    {title:"Redis",icon:Gauge,status:data?.redis?.status,detail:data?.redis?`${data.redis.latencyMs} ms · limites distribuídos`:""},
  ...
  return <div className="ds-page space-y-4">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{cards.map(card=>{const Icon=card.icon;const ok=card.status==="operational";return <article key={card.title} className="ds-card p-5"><div className="flex items-start justify-between"><span className="grid h-10 w-10 place-items-center rounded-xl bg-blue-50 text-blue-600"><Icon size={20}/></span>{loading&&!data?<RefreshCw className="animate-spin text-slate-400" size={18}/>:ok?<CheckCircle2 className="text-emerald-600" size={19}/>:card.status==="unavailable"?<XCircle className="text-red-600" size={19}/>:<AlertTriangle className="text-amber-600" size={19}/>}</div><h3 className="mt-4 font-black">{card.title}</h3><span className={`ds-badge ds-status--${tone(card.status)} mt-2`}>{label(card.status)}</span><p className="mt-3 text-xs text-slate-500">{card.detail}</p></article>})}</div>
    {data?.requests?<section className="ds-card grid gap-px overflow-hidden sm:grid-cols-2 lg:grid-cols-4"><Metric label="Requisições (5 min)" value={data.requests.last5Minutes.requests}/><Metric label="Latência P50" value={`${data.requests.last5Minutes.latencyP50Ms} ms`}/><Metric label="Latência P95" value={`${data.requests.last5Minutes.latencyP95Ms} ms`}/><Metric label="Erros 5xx" value={data.requests.last5Minutes.errors5xx} danger={data.requests.last5Minutes.errors5xx>0}/></section>:null}
...
function Metric({label,value,danger=false}:{label:string;value:string|number;danger?:boolean}){return <div className="border-b border-r p-4"><span className="text-[10px] font-black uppercase tracking-wide text-slate-500">{label}</span><strong className={`mt-1 block text-xl ${danger?"text-red-600":"text-slate-950"}`}>{value}</strong></div>}
