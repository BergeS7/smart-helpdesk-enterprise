...
// Pódio revelado de baixo para cima: 3º, depois 2º e por último o campeão.
const podiumDelay: Record<number, number> = { 3: 150, 2: 350, 1: 600 };
    ...
    <div className="p-4 sm:p-6">
      ...
      {!loading && !error && ranking.length > 0 && <>
        <section className={`relative overflow-hidden rounded-3xl bg-gradient-to-br p-5 text-white sm:p-7 ${dark ? "from-indigo-950 via-indigo-900 to-violet-950" : "from-indigo-700 via-indigo-600 to-violet-600"}`}>
          ...
          <div className="relative mx-auto mt-6 grid max-w-3xl grid-cols-3 items-end gap-2 sm:gap-4">{podium.map((item) => { const position = ranking.indexOf(item) + 1, first = position === 1; return <article key={item.technician_id || item.id || item.name} style={enter(podiumDelay[position] ?? 0)} className={`motion-podium flex flex-col items-center rounded-t-3xl border border-white/15 bg-white/10 px-2 pb-4 text-center backdrop-blur-sm ${first ? "min-h-60 pt-4" : "min-h-48 pt-5"}`}>{first && <Crown size={22} style={enter(1050)} className="motion-crown mb-2 fill-amber-300 text-amber-300"/>}<span className={`grid place-items-center rounded-full border-4 border-white/30 bg-white font-black text-indigo-700 shadow-xl ${first ? "h-20 w-20 text-xl" : "h-16 w-16 text-base"}`}>{initials(item.name)}</span><b className="mt-3 line-clamp-2 text-sm">{item.name || "Profissional"}</b><span className="mt-1 text-[10px] text-white/60">{item.departamento || "Equipe técnica"}</span><span className="mt-auto pt-3 text-2xl font-black">{position}º</span><span className="mt-1 flex items-center gap-1 text-xs font-black text-amber-200"><Star size={13} className="fill-current"/><CountUp value={Number(item.average_rating)} decimals={1} duration={1200}/></span></article>; })}</div>
        </section>
        <section className="mt-4"><div className="mb-3 flex items-end justify-between gap-3"><div><h3 className="text-sm font-black">Classificação completa</h3><p className={`mt-0.5 text-[11px] ${muted}`}>{ranking.length} profissional(is) no período</p></div><span className={`flex items-center gap-1.5 text-[10px] ${muted}`}><ShieldCheck size={13} className="text-emerald-600"/>Mínimo de {minimumSample} avaliações</span></div><div className="space-y-2">{ranking.map((item, index) => <article key={item.technician_id || item.id || item.name} style={enter(900 + Math.min(index, 15) * 60)} className={`motion-enter grid grid-cols-[42px_44px_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border p-3 ${card}`}><span className={`grid h-9 w-9 place-items-center rounded-xl text-xs font-black ${index === 0 ? "bg-amber-100 text-amber-700" : index === 1 ? "bg-slate-200 text-slate-700" : index === 2 ? "bg-orange-100 text-orange-700" : dark ? "bg-white/10 text-slate-300" : "bg-zinc-100 text-zinc-600"}`}>{index < 3 ? <Medal size={16}/> : `${index + 1}º`}</span><span className="grid h-11 w-11 place-items-center rounded-full bg-indigo-50 text-xs font-black text-indigo-700">{initials(item.name)}</span><div className="min-w-0"><b className="block truncate text-sm">{item.name || "Profissional"}</b><span className={`mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[10px] ${muted}`}><span className="flex items-center gap-1"><MessageSquare size={11}/>{item.total_ratings} avaliações</span><span>SLA {Number(item.sla_rate || 0).toFixed(0)}%</span></span></div><span className="flex items-center gap-1 text-base font-black"><Star size={15} className="fill-amber-400 text-amber-400"/><CountUp value={Number(item.average_rating)} decimals={1}/></span></article>)}</div></section>
      </>}
    </div>
  </section>;
}
