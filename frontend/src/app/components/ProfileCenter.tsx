      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[235px_1fr]">
        ...
        <main className="min-h-0 overflow-auto p-5 sm:p-6">
          <div key={section} className="settings-section-enter">
            ...
            {section === "privacidade" && <ProfileSettings title="Privacidade e LGPD" description="Transparência e controle sobre os seus dados." panel={panel} muted={muted}><Info icon={<Eye size={18}/>} title="Dados utilizados" text="Nome, e-mail, contato, cargo, departamento, foto e registros necessários para autenticação, triagem e auditoria dos chamados."/><Info icon={<ShieldCheck size={18}/>} title="Finalidade" text="Os dados são utilizados somente para operação, segurança, suporte e rastreabilidade do atendimento."/><div className="flex flex-wrap gap-2"><button type="button" onClick={exportData} className="inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-bold text-white transition hover:bg-blue-700"><Download size={16}/>Baixar meus dados</button><button type="button" onClick={() => openLegalDocument("privacy")} className={`inline-flex h-11 items-center gap-2 rounded-xl border px-4 text-sm font-bold transition hover:bg-blue-50 hover:text-blue-700 ${dark ? "border-white/10" : "border-zinc-200"}`}><FileText size={16}/>Política de Privacidade</button><button type="button" onClick={() => openLegalDocument("cookies")} className={`inline-flex h-11 items-center gap-2 rounded-xl border px-4 text-sm font-bold transition hover:bg-blue-50 hover:text-blue-700 ${dark ? "border-white/10" : "border-zinc-200"}`}><Cookie size={16}/>Política de Cookies</button></div><p className={`text-xs ${muted}`}>Para corrigir ou solicitar exclusão de dados corporativos, entre em contato com o administrador responsável.</p></ProfileSettings>}
          </div>
        </main>
      </div>
    </aside>
