...
import { Bell, Camera, Check, ChevronRight, Cookie, Download, Eye, FileText, LogOut, MonitorCog, Palette, ShieldCheck, Trash2, UserCog, X } from "lucide-react";
import { listarCatalogo, type ApiUsuario } from "../services/api";
import { PushNotificationSettings } from "./PushNotificationSettings";
import { openLegalDocument } from "./LegalComplianceLayer";
import { MyAssetsCard } from "./patrimonio/MyAssetsCard";
      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[235px_1fr]">
        ...
        <main className="min-h-0 overflow-auto p-5 sm:p-6">
          <div key={section} className="settings-section-enter">
            ...
            {section === "privacidade" && <ProfileSettings title="Privacidade e LGPD" description="Transparência e controle sobre os seus dados." panel={panel} muted={muted}><Info icon={<Eye size={18}/>} title="Dados utilizados" text="Cadastro, acesso, chamados, mensagens, anexos, avaliações e, quando aplicável, inventário dos equipamentos corporativos."/><Info icon={<ShieldCheck size={18}/>} title="Finalidade" text="Operação do suporte, segurança, auditoria, inventário e indicadores de atendimento, conforme a política de privacidade."/><div className="flex flex-wrap gap-2"><button type="button" onClick={exportData} className="inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-4 text-sm font-bold text-white transition hover:bg-blue-700"><Download size={16}/>Baixar dados do perfil</button><button type="button" onClick={() => openLegalDocument("privacy")} className={`inline-flex h-11 items-center gap-2 rounded-xl border px-4 text-sm font-bold transition hover:bg-blue-50 hover:text-blue-700 ${dark ? "border-white/10" : "border-zinc-200"}`}><FileText size={16}/>Política de Privacidade</button><button type="button" onClick={() => openLegalDocument("cookies")} className={`inline-flex h-11 items-center gap-2 rounded-xl border px-4 text-sm font-bold transition hover:bg-blue-50 hover:text-blue-700 ${dark ? "border-white/10" : "border-zinc-200"}`}><Cookie size={16}/>Política de Cookies</button></div><p className={`text-xs ${muted}`}>O arquivo contém apenas cadastro e preferências deste perfil. Para acesso aos demais dados ou para solicitar correção, oposição ou eliminação nas hipóteses legais, procure o administrador responsável pela privacidade na empresa.</p></ProfileSettings>}
          </div>
        </main>
      </div>
    </aside>
