...
function SolicitanteAvatar({chamado,size="md"}:{chamado:ApiChamado;size?:"sm"|"md"|"lg"}){const nome=nomeSolicitanteChamado(chamado),tamanho={sm:"h-7 w-7 text-[10px]",md:"h-9 w-9 text-xs",lg:"h-10 w-10 text-sm"}[size];return <span title={nome} className={`grid shrink-0 place-items-center overflow-hidden rounded-full border border-zinc-200 bg-gradient-to-br from-blue-500 to-sky-400 font-black text-white shadow-sm ${tamanho}`}>{chamado.solicitante_foto_url?<img src={chamado.solicitante_foto_url} alt={nome} className="h-full w-full object-cover"/>:iniciais(nome)}</span>}
