...
export function deriveDeviceAlerts(devices:Device[]):DeviceAlert[]{return devices.flatMap((d)=>{const rows:DeviceAlert[]=[];const add=(suffix:string,tipo:string,severidade:DeviceAlert["severidade"])=>rows.push({id:`${d.id}-${suffix}`,tipo,deviceId:d.id,hostname:d.hostname,municipio:d.municipio,horario:d.ultimoHeartbeat,severidade});if(d.status==="offline")add("offline","Computador diagnosticado como offline","critical");if((d.diskUsage??0)>=85)add("disk","Espaço em disco crítico","critical");if((d.ramUsage??0)>=85)add("ram","Alto uso de memória","warning");if((d.cpuUsage??0)>=85)add("cpu","Alto uso de CPU","warning");if(d.antivirusAtualizado===false)add("av","Antivírus desatualizado","warning");return rows;});}
export async function searchAssignableUsers(query:string):Promise<AssignableUser[]>{return assetRequest<AssignableUser[]>(`/admin/users?q=${encodeURIComponent(query)}`);}
export async function setDeviceUser(id:string,usuarioId:number|null):Promise<Device>{return assetWrite<Device>(`/${id}/user`,"PATCH",{usuario_id:usuarioId});}
export async function getMyAssets():Promise<MyAsset[]>{const response=await fetch(`${API_URL}/usuarios/me/ativos`,{headers:{Authorization:`Bearer ${getToken()||""}`}});const data=await response.json().catch(()=>null);if(!response.ok)throw new Error(data?.erro||"Erro ao consultar seus ativos");return data as MyAsset[];}
